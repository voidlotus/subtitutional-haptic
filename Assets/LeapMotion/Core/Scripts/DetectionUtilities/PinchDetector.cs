...
namespace Leap.Unity {
  ...
  public class PinchDetector : AbstractHoldDetector {
    ...
    public float GetPinchStrengthValue { get {return strengthValue; } }
    ...
    protected override void OnDrawGizmos () {
      if (ShowGizmos && _handModel != null && _handModel.IsTracked) {
        ...
        if (_handModel.GetLeapHand().PinchStrength > 0.0f)
        {
            
            strengthValue = _handModel.GetLeapHand().PinchStrength;
            Debug.Log("pinchDetector: "+strengthValue);
        }
